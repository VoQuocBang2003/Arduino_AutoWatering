...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin("http://192.168.2.19/ESP32_MySQL_Database/getdata.php");  //nhập ip
    ...
    if (mySerial.available() > 0) {
      ...
      l = getValue(data, ';', 2).toFloat();
      s = getValue(data, ';', 3).toFloat();
      Serial.println("T=" + String(t) + "\tH=" + String(h));
      Serial.println("l=" + String(l) + "\ts=" + String(s));
    }
    delay(1000);
    ...
    postData += "&soil=" + String(s);
    postData += "&light=" + String(l);
    ...
    http.begin("http://192.168.2.19/ESP32_MySQL_Database/updateDHT11data_and_recordtable.php");  //nhập ip
    http.addHeader("Content-Type", "application/x-www-form-urlencoded");  //--> Specify content-type header
